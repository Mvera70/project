...
/**
 * When the village was last asked a question of §8.6's kind.
 *
 * §7.8, v2.97: trade is not of that kind and is excluded here. The traders have
 * a channel and a clock of their own precisely so that they do not spend this
 * rest — a pedlar arriving must not push back the succession that a death has
 * made due. `lastTradeTick` below is their half of the same idea.
 */
export function lastCrossroadTick(state: GameState): number {
  let last = -1;
  for (const d of state.history) {
    if (isTrade(d.templateId)) continue;
    if (d.tick > last) last = d.tick;
  }
  if (state.crossroad !== null
    && !isTrade(state.crossroad.templateId)
    && state.crossroad.posedTick > last) {
...
/** When a trader was last at the door, on the traders' own clock. */
export function lastTradeTick(state: GameState): number {
  let last = -1;
  for (const d of state.history) {
    if (isTrade(d.templateId) && d.tick > last) last = d.tick;
  }
  if (state.crossroad !== null
    && isTrade(state.crossroad.templateId)
    && state.crossroad.posedTick > last) {
...
export function eligible(state: GameState, catalogue: Catalogue): ScoredTemplate[] {
  ...
  for (const t of catalogue) {
    ...
    // §7.8, v2.97: the traders never enter this draw. They are picked by
    // `selectTrader` on their own clock, so that adding one does not take a
    // turn away from a famine.
