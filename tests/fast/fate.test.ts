...
describe('los sucesos del valle · R-1', () => {
  ...
  it('el mundo sigue en pie con los sucesos dentro', () => {
    // Medido en la primera vuelta: el rayo quemaba la única casa de la pareja y
    // tres aldeas de seis morían antes del año treinta. La puerta de
    // `LIGHTNING_MIN_HOUSES` lo cierra, y esto lo vigila.
    for (const [i, state] of worlds.entries()) {
      expect(state.ended, `semilla ${SEEDS[i]}`).toBeNull();
