// Banco de gestos de un animal sobre su GLB (el publicado u otro) y el mismo
// `Fauna` del juego. Lo monta `animal-gestures-bench.mjs`.
...
declare const B: Record<string,string>; declare const M: AssetManifest; declare const KIND: string; declare const FRAME: number;
...
const s=FRAME;const cam=new OrthographicCamera(-s,s,s*.87,-s*.87,.01,100);
...
window.frame=(action,t,moving)=>{
  ...
  while(time<t-1e-8){const dt=Math.min(1/60,t-time);time+=dt;if(moving)x-=(action==='run'||action==='charge'?1.4:.6)*dt*s*.75;fauna.paint([{id:1,kind:KIND as never,x,y:0,action:action==='idle'?undefined:action as never}],time);}
  cam.position.set(x-s*.5,s*1.4,s*3.5);cam.lookAt(x,s*.3,0);r.render(scene,cam);return r.domElement.toDataURL('image/png');
};
window.ready=true;
