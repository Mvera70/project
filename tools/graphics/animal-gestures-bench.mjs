...
//     [--actions idle,walk,run,bark,play] [--frame 0.4]
// Sin `--glb`, el publicado. Escribe artifacts/graphics/animal-gestures/<kind>-gestures.png.
const arg=(n,d)=>{const i=process.argv.indexOf('--'+n);return i>0?process.argv[i+1]:d;};
const KIND=arg('kind','dog'),GLB=arg('glb',null),FRAME=Number(arg('frame','0.4'));
const ACTIONS=arg('actions',KIND==='dog'?'idle,walk,run,bark,play':'idle,walk').split(',');
const dir='artifacts/graphics/animal-gestures';mkdirSync(dir,{recursive:true});const a='public/assets/valley3d/';const all=JSON.parse(readFileSync(a+'manifest.json'));
const M={...all,assets:all.assets.filter(x=>x.id===KIND).map(x=>GLB===null?x:{...x,motion:JSON.parse(arg('motion',JSON.stringify(x.motion)))})};
...
await p.evaluate(()=>Promise.all([...document.images].map(i=>i.decode())));await p.screenshot({path:dir+'/'+KIND+'-gestures.png',fullPage:true});await br.close();console.log('ok');
