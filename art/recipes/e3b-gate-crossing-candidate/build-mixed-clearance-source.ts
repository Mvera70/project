...
const dir='art/recipes/e3b-gate-crossing-candidate',out='artifacts/graphics/E3b2-candidates/gate-mixed-source-review-03';await mkdir(out,{recursive:true});
...
const frame=structuredClone(wide);frame.id='e3b-gate-wide-clearance-candidate';
const lintelPrimitive=frame.primitives.find((p:{name:string})=>p.name==='Gate_Stone_Lintel');
lintelPrimitive.location[2]=2.025;lintelPrimitive.dimensions[2]=.27;
frame.metadata.note='Marco amplio con dintel Y0.63…0.72. Hoja, gozne, jambas y escala idénticos al marco amplio anterior. Sólo fuente.';
...
for(const mask of [65,24]){
 ...
 const parts:Part[]=[prism('Deck',floor,.94,1.02,'#CDC9C2'),prism('NorthAbutment',north,0,.63,'#CDC9C2'),prism('SouthAbutment',south,0,.63,'#CDC9C2'),prism('PortalLintel',floor,.63,.94,'#CDC9C2')];
 ...
 for(let i=0;i<position.count;i++){const v=new Vector3().fromBufferAttribute(position,i).applyMatrix4(lintelMesh.matrixWorld);v.y=lb.max.y-(lb.max.y-v.y)*.75;v.applyMatrix4(inverse);position.setXYZ(i,v.x,v.y,v.z);}
