...
8. **La parada del 17 sep: el juego de los medios.** «Ahora mismo no es nada
   ...
   `docs/historico/plan-medios.md`: las palancas de órdenes son una trampa, las
...
1. **Todo lo dramático del motor colgaba de las encrucijadas.** Las opiniones
   ...
   (`docs/medidas/findings-drama.md`, medido tres veces).
...
| `tools/reports/fate-report.ts` | El informe que fijó los pesos: `npx tsx tools/reports/fate-report.ts [semillas] [--years 40]` | nuevo |
...
`npx tsx tools/reports/fate-report.ts 3 7 11 23 31 41 53 67 79 83 89 97` con doce
...
- **Playwright y la demo.** `npm run test:shots` no se volvió a pasar tras R-1 y la demo
  ...
  Artifact con esa URL (`docs/historico/next-plan.md` §4). Y la **secuencia de capturas
  ...
  `tools/reports/fate-report.ts` o un guion de tres líneas, y llega hasta él con
...
Escribe `tools/reports/life-report.ts` (o una prueba en `tests/fast/life-motion.test.ts`
...
**Criterio.** `tools/reports/fate-report.ts` con seis semillas: la distancia media
...
**De dónde sale.** De la parada del 17 sep: `docs/historico/plan-medios.md` es el
...
>
> **Medido** (`tools/reports/agency-report.ts`, 32 semillas × 60 años): la primera
...
Recorrido nuevo en `tools/shots/valley.shots.ts`: una oferta se lee en la voz, se acepta
con un toque y la plata sube en la cabecera.

**Medida y terminado.** Con reposo, sesenta años, dieciséis semillas (el
script de `docs/historico/plan-medios.md` §1, que hay que meter en `tools/` como
`tools/reports/agency-report.ts` porque las cinco fases lo usan): la piedra llega en los
...
**Medida y terminado.** `tools/reports/agency-report.ts`: con reposo y sin medios, **las
...
> `tools/reports/agency-report.ts`, 24 semillas × 60 años:
>
...
2. **Lo que cada uno hace en el motor**, y sus dos sucesos:
   ...
   - **El barril.** Una fiesta **esa semana**: `happen('ale_feast')` directo
     ...
     que da al ánimo el reloj del jugador (`docs/historico/plan-medios.md` §6.2).
...
(el carro sustituye a las órdenes, no se suma). Recorrido en `tools/shots/valley.shots.ts`:
...
`tools/reports/agency-report.ts` con **tres combinaciones fijas** (sólo arado en el año 3;
...
`docs/historico/plan-medios.md`. Secuencia de capturas: el carro, el barril dado, la fiesta,
...
**Qué.** Los medios que quedan de `docs/historico/plan-medios.md` §3.3 —el hacha, el forastero
...
| Fase | Estado | Dónde |
|---|---|---|
| R-1 | **hecho, v3.75, en `main`** (queda §2.6 y la captura de §2.8) | `src/engine/world/fate.ts`, `sky.ts`; `tools/reports/fate-report.ts`; §4.2 2b, §7.10, §12.10 |
| §3 IA · puntos 1, 3, 6 | **hecho, v3.76** — paredes 1,31 % → 0,10 %, vueltas 4,75 % → 0,34 % (§3.6) | `life/body.ts`, `steering.ts`, `navigate.ts`, `beasts.ts`, `terrain.ts`, `village.ts`, `tools/reports/life-report.ts` |
