...
(`tools/shots/valley.shots.ts`, `advanceWeeks`). Con eso se fotografía el juego de
...
llegan a los cuarenta años son 11, 53, 67 y 79 (`docs/historico/rework.md` §2.5).
...
- **R-1 de punta a punta, y se ve.** El fotograma 2 tiene a la aldea en corro en
  ...
  como reunión. Es exactamente lo que `docs/medidas/findings-drama.md` decía que no pasaba
  nunca.
...
- **La secuencia de cuatro tiempos que el brief pide por fase** —aproximación,
  ...
  tick en `state.happenings` con `tools/reports/fate-report.ts` y se adelanta a él.
