...
1. `src/engine/balance.ts` · `TIME.REAL_MS_PER_TICK` 15 000 → 840 000, con su
   ...
   `tests/journeys/…` que cuenten ticks por segundo, y en `tools/shots/valley.shots.ts`
...
5. `src/ui/app.ts` · sustituye `year` + `season` por un `clock` con cuatro
   ...
   (`grep -n "ANNO" tools/shots/valley.shots.ts`).
...
`tools/shots/valley.shots.ts`; una es `test.fail()` declarada (ver su cabecera).
...
`tools/reports/sky-report.ts` los midió (79 % de jornadas claras, una tormenta cada tres
...
6. **Ruta de depuración y captura.** `src/ui/debug.ts` (`stateAt`,
   ...
   `weatherAt` diga tormenta. Añade a `tools/shots/valley.shots.ts` un recorrido que
...
«la crónica y la gente se abren y se cierran» de `tools/shots/valley.shots.ts`.
...
derivándolos de la constante (`msFor`, `advanceWeeks` en `tools/shots/valley.shots.ts`),
...
1. ~~**La cámara**~~ — **hecha, G-13.** Gira con dos dedos o con mayúsculas, se
   ...
   `docs/historico/graphics-rounds/G-13.md`, con tres capturas desde tres ángulos.
...
suite de recorridos y las tablas de `docs/historico/life-rounds/`. Si hace falta una sonda
...
desconfianza, remedir. Escribe su informe en `docs/historico/life-rounds/<ronda>-audit.md`
