...
> tiene su sala y **se ve en la lista**. Lo medido está en `docs/medidas/rey-medida.md`;
...
**Para quien lo ejecute.** 18 sep 2026. Brief de fase con la forma de `docs/historico/rework.md` §4b (M-0 a M-4): qué toca cada paso, el contrato literal, las pruebas como propiedades y el criterio de terminado. Se lee después de `CLAUDE.md`, `docs/design.md` §1–4, §6, §7.2–7.3, §7.12, §8, §12, §13 y `docs/task-log.md` §4.0.
...
**Ficheros.** `tools/reports/agency-report.ts` (sólo columnas nuevas), `docs/medidas/rey-medida.md` (nuevo, la tabla).

**Cómo.** Añadir a `Row` y a la tabla: `walls` (tramos de `palisade` + `wall` en pie), `faith` (fe al final), `chapelYear` (año de la primera capilla), `lordAsked` (decisiones de `history` cuya plantilla es de categoría `lord`), `events` (`state.happenings.length`). Correr `npx tsx tools/reports/agency-report.ts --seeds 24 --years 60 --only nada,arado,reliquia,"peor encrucijada"` y `npx tsx tools/reports/fate-report.ts` con las seis semillas de siempre. Guardar las filas en `docs/medidas/rey-medida.md` §1 tal cual salen.
...
- Recorrido en `tools/shots/valley.shots.ts` contra el render 3D (dice contra cuál corre, regla de `CLAUDE.md`): abrir el carro en un valle que puede coronar (`?crown=ready` en la ruta de depuración), tocar «Crown» en el segundo candidato, la voz dice la coronación, la lista de la gente lo llama `king`, y la fila del carro pasa a «reigns». Captura en `artifacts/k5-crown.png`.
...
**Ficheros.** `tools/reports/agency-report.ts` (cinco variantes: `rey herrero`, `rey del campo`, `rey cura`, `rey de corte`, `rey cualquiera`; cada una corona al primer candidato de ese estilo la primera semana que `crownRefusal` lo permite y no da nada más; y `arado y rey herrero` para ver si se complementan), `docs/medidas/rey-medida.md` §2, `docs/design.md` §12 (la tabla de `CROWN` con lo medido al lado), `docs/task-log.md`.
...
**Listones de §12.9 que esto puede mover**, y hay que anotar en `docs/medidas/rey-medida.md` con el número: «Mediana del pico de población 65–85» (`plough` hacia arriba, `forge` hacia abajo por las manos en la muralla); «Partida terminada 2–12 %» (`FORGE_LORD` y `watched` traen más señor); «Encrucijadas por generación 1–5» (más `lord` con `forge`/`court`); «Fracción de ticks elegibles < 1 %» (las plantillas del señor con `watched` puesto veinte años: correr `npm run eligibility` con `--crown court`). La suite de balance está por remedir desde R-1 (decisión 5): estas cifras van al informe, no a los asertos.
...
- **Hierro, armas, un quinto montón:** no. «Si el rey herrero hace armas, las armas son un rasgo de valle y no un quinto montón» (`docs/historico/plan-medios.md` §6.3): aquí son la muralla adelantada y el señor que la cuenta.
...
|---|---|---|---|
| K-0 | — | `tools/reports/agency-report.ts`, `docs/medidas/rey-medida.md` | — |
...
| K-5 | `chronicle/bank.en.ts`, `bank.es.ts` | `derive/crown.ts`, `ui/redesign/contracts.ts`, `cart.ts`, `hud.ts`, `people-panel.ts`, `ui/person-card.ts`, `ui/inspect.ts`, `ui/app.ts`, `tools/graphics/shot.mjs`, `observe-life.mjs`, `tools/shots/valley.shots.ts` | `crown.test.ts`, `tools/shots/valley.shots.ts` |
| K-6 | `balance.ts` (comentarios) | `tools/reports/agency-report.ts`, `docs/medidas/rey-medida.md`, `docs/design.md` §12, `docs/task-log.md` | — |
