...
| ~~A3 · El bastión~~ · **hecho el 19 sep 2026**: la mitad de esta fila que no choca con §7.4c. Torre **en la línea** de muralla y no al lado, **1×1** y no 2×2 —sobre un cerco de una celda una torre de dos tapaba dos o tres tramos y `upgradeOf` sólo daba de baja uno, un boquete con `ringClosed` diciendo cerrado—. Es una mejora más (`upgradeOf: 'wall'`, como `stone_house`/`church`), sólo se pide con el anillo ya cerrado (`flags['wall_closed']`, no `ringClosed(state)` en directo — recorrer la rejilla cada semana ociosa es justo lo que esa bandera existe para evitar) y con tope propio (`BUILDINGS.bastion.cap`, contado a mano porque `withinCap` colapsa su familia en `'wall'`, que no tiene tope). Cuenta como muralla en `wallRuns`, `touchesWall`, `resistance` y `walled`; ocupa un puesto de tiro en `postsOf` igual que la atalaya suelta. Sin malla propia todavía —usa la de la atalaya, anotado en `docs/encargos-3d.md`— y sin dibujo de crónica propio —anotado en `docs/plan-arte-pendiente.md`, cae al grabado genérico de construcción mientras tanto | Hecho | Media | — | A1 |
| A3b · El segundo anillo | **Contradice una decisión medida del dueño del diseño, y no se ha resuelto — no la toma ningún agente.** Esta fila pedía «cuándo la aldea desborda el primero y pide el siguiente tres celdas afuera», y `design.md` §7.4c prohíbe exactamente eso desde el 18 sep 2026, el mismo día que esta fila se escribió: «un anillo por valle, y uno solo… no se levanta un segundo anillo alrededor del primero», medido con 1 824 tramos de muralla contra 131 casas en doce semillas a 120 años y una semilla con anillos en 8, 11, 14 y 17 celdas para catorce casas. Antes de tocar esto hace falta que el dueño diga si quiere el segundo anillo de verdad —y entonces §7.4c se reescribe con su nueva medida— o si lo que quería esta fila ya lo da el bastión de A3 | P4 | — | — | A3 |
...
- **Si el valle levanta un segundo anillo de muralla** (A3b): §7.4c lo prohíbe
  por una medida del propio dueño (18 sep 2026), y esta fila pedía justo lo
  contrario. No se resuelve solo porque el plan lo pida.
