...
| E4 · Fuego, humo, gore · **en curso, 25 sep 2026** | **Decidido por el dueño el 25 sep:** gore *contenido* (salpicadura corta al golpe y mancha que se desvanece), fuego en las cuatro formas (los incendios que ya había, casas quemadas en el asalto, flechas incendiarias, y que dure días) y llamas de *textura dibujada animada*. **Tanda 1 hecha:** la casa quemada por el incendio anual o el rayo arde en pantalla tres días de llama y cuatro de brasa (`BURNING`), con humo negro, chispas y luz; el motor lo marca con `burnt:<id>` (`burnBuilding`), sin campo nuevo ni azar. **Tanda 2 hecha:** gore contenido —cinco gotas al recibir un golpe o caer, y una mancha oscura bajo el cuerpo que se va en ocho horas escénicas (`GORE`, `effects/stains.ts`)—, probado con el reparto real; en partida la mancha no se distingue bien bajo el velo del bosque del observatorio. **Tanda 3 hecha:** tomada la villa, arden las tres casas de madera más cercanas al portón (`THREAT.STORM_BURN`); saqueada una aldea abierta, una (`SACK_BURN`) y nunca la última; tras la muralla, ninguna. Línea `raid.burnt` con su imagen pedida. **Falta:** las flechas incendiarias con la defensa apagando (4) | P4 | Alta | — | D5, D6 |
