...
|---|---|---|---|
| **3.65** | 14 sep 2026 | `findings-drama.md` §7, los dos fallos | **`wolf_winter` era inalcanzable por construcción, igual que A.11 antes de ella.** Exigía `forestLeft > 0.25`; el valle funda entre 0.20 y 0.26 (`MAPGEN.FOREST_FRACTION`) y sólo tala desde ahí, así que nunca llegaba al umbral — medido 0.177–0.244 en cuatro semillas × sesenta años, cero apariciones. Bajado a 0.15, con `forestLeft` intacto: es una **puerta** de bosque suficiente, no un filtro de frecuencia, y cambiarle el significado habría movido el `forage` de M-06 y `forest_cut` que ya lo usan. Detalle en A.12. **`tithe_demand`, investigado y no tocado:** la cadena que debía levantar `vassal` sí está escrita — `winter_grain_debt.kneel` pone `flag vassal years:0`, verificado disparando la opción a mano — y sí funciona; lo raro no es la bandera, es que `winter_grain_debt` casi nunca es elegible (`grainToHarvest < 0.9` en pleno invierno) y sólo una de sus tres opciones vota vasallaje. Ya estaba anotado como «por diseño, gira sobre una decisión anterior» en la auditoría de v2.46 (§ tabla de la aldea madura) y con la política `worst`, que nunca se arrodilla, da 0 → 0. No es un bug: es la pregunta de ritmo de `docs/roadmap.md` §1, y no se toca aquí. |
...
**Requiere** `season = winter`, `forestLeft > 0.15`, `people > 15` — el 0.25 original era **inalcanzable**, la misma errata que el 0.3 de A.11: `forestLeft` es bosque sobre el mapa entero, `MAPGEN.FOREST_FRACTION` funda el valle entre 0.20 y 0.26 y sólo baja desde ahí, así que nunca cruzaba el umbral — medido 0.177–0.244 en cuatro semillas × sesenta años, cero apariciones. El bosque es aquí, como en A.11, una **puerta** («sin bosque no hay lobos»), no el disparador episódico, que ya lo dan `season = winter` y `people > 15`. Bajado a 0.15, por debajo del suelo medido en esa ventana (`tools/eligibility-report.ts`, 14 sep 2026)
