...
- La repetición final de Knip no deja código muerto confirmado: sólo enumera
  ...
  consumidos por `tools/works-report.ts`, y `RecipeClip`, consumido por la
...
- `tools/subpath-server.mjs` y `tools/stale-server.mjs` son servidores declarados
...
- `canQuarry` y `woodCostOf`, aunque Knip los marcó al excluir el CLI no
  alcanzable, sí se importan desde `tools/works-report.ts` y no son API muerta.
