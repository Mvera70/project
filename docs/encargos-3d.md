...
| **Hacha y pico** (IA-anim, 24 sep 2026) | Talar y picar piedra | Respaldo procedimental en `hand-tools.ts`: mango de 1 m, cabeza exagerada, orientados al marco de la mano medido en el banco de gestos. Un GLB propio con conector `grip` los sustituiría sin tocar los gestos (`HELD.chop` → `axe`, `HELD.mine` → `pickaxe`) |
