...
| **El cuerpo a cuerpo** (D4) | Quien llega al alcance de un brazo golpea a quien defiende, y el que defiende devuelve; de 0 a 3 de los nuestros caen por asalto | **Nada.** Los dos se pintan con `idle`, quietos y de pie, dándose golpes invisibles. Y el que cae se queda plantado donde cayó | Los tres clips de E1 (`spear_thrust`, `hit_take`, `fall`) y **el ragdoll**, que no es un clip: pide que los cuerpos de la gente sean cuerpos de Rapier, y eso es una tanda entera. Ver `docs/encargos/encargo-combate.md` |
...
| Qué | Estado |
|---|---|
| **Clips de combate** (E1) | **No existe ni uno**, y es el camino largo del proyecto. **Encargo escrito el 18 sep: `docs/encargos/encargo-combate.md`** — seis clips sobre el aparejo del aldeano que ya existe, con sus nombres, duraciones y el orden en que hacen falta. Con los tres primeros (tensar, soltar, caer) ya se puede hacer una defensa que se mire |
