...
| **El bastión** (`bastion`, A3, 19 sep 2026) | La torre en la línea de muralla | **Modelo candidato hecho, no integrado:** `art/recipes/bastion/bastion.json`, validación G-26; base 1×1, plataforma abierta, 1.092 triángulos y dos materiales. El juego todavía usa `watchtower.glb` escalada. Falta aprobación visual, publicar/admitir el GLB y corregir por separado `isDefence`/`defenceConnections` (`render3d/world/defences.ts`), que no conectan el muro al bastión; también queda el puesto elevado navegable. [Brief de modelo](encargos/encargo-e3-modelo-bastion.md) |
