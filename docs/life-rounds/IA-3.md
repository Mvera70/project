...
`tools/life-traits-report.ts` (nuevo, fuera de los ficheros autorizados de
`life/`: es una herramienta de medida, como `tools/life-report.ts` de IA-1, no
un cambio de comportamiento) sale de la misma disciplina que
`tools/life-report.ts`: nunca `tick` en un bucle, siempre `foundTwenty` +
...
`npx tsx tools/life-traits-report.ts 7 23 41 67 79 97 --days 3 --years 30`
...
`npx tsx tools/life-report.ts` suelto medía una `beasts.ts` distinta cada vez
...
**Y una segunda, de medición**: la primera versión de `tools/life-report.ts`
