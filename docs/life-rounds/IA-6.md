...
- **El funeral y el incendio no se han construido.** `villager.diedTick` da el
  ...
  `docs/rework.md` ya los tenía planeados con los cubos de `props.ts` y el humo
...
Las dos rojas de jornadas son las dos del lote de nueve de `rework.md` §2.8, y
...
`state.happenings`, no con `tools/fate-report.ts`, que da totales por semilla y
