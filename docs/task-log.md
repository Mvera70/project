...
|---|---|---|
| **El paquete de prensa, a medias: 64 capturas de unas 108** · pedido el 18 sep 2026 para un vídeo tráiler, **suspendido por el dueño el 19 sep** | Quien retome, con `tools/graphics/press-kit.mjs` | Está en `artifacts/graphics/press/` con su hoja de contactos (`index.html`, cada captura con su pie, su hora y su pantalla). Hecho: las pantallas con interfaz (menú, vuelo de entrada, el valle con regleta/pausa/sin sonido/despejado/de cerca, cuatro horas, crónica, gente, ficha, siguiendo, carro, encrucijada, sello, víspera/saqueo/asalto, dos finales) y el metraje sin interfaz (nueve edades del valle, tres estaciones, siete escenas). **Lo que falta, con su comando**: `--offset 62 --only escenas` (tres escenas), `--offset 70 --only cerco`, `--offset 76 --only asedio,estados,lapidas`, y **rehacer `--offset 37 --only crecimiento`** porque la tira que se envió tiene la hora descuadrada y el año 32 con la interfaz puesta (las dos causas ya están arregladas en la herramienta: `atHour` fija la hora a ×64 y `bare` reintenta). **Y el encuadre es vertical** (1170 × 2532, móvil a 3×): si el tráiler va horizontal, `--width 1280 --height 720`. El guion del tráiler que el dueño escribió está en su mensaje del 18 sep — y dos de sus planos **no existen en el juego**: un valle literalmente vacío (se abre con la pareja ya puesta) y un entierro (no hay clip; la muerte se cuenta en la crónica y deja tumba) |
| **Una crónica enseña `{B}` en vez de un nombre** · cazado en una captura de UI-V10 | Sol (motor) | En la semilla 7 al año 50 se lee **«The smithy passed to {B} in year 49.»**, con la llave y la letra en pantalla. La causa está identificada y no es de la interfaz: `namesOf` (`crossroads/resolve.ts`) se salta a quien tiene el nombre vacío, y el valle tiene **42 personas con 6 nombradas**, así que una encrucijada cuyo reparto caiga en alguien sin nombre escribe la plantilla con el hueco sin rellenar. Los muertos no son el problema —«includes the dead; nobody is ever removed»—, es la gente anónima. Lo que hay que decidir es cuál de las dos: que el reparto sólo coja a gente nombrada, o que ser elegido para una encrucijada **nombre** a esa persona (que es lo que el juego ya hace cuando alguien se vuelve notable). Es una línea de peso 3 y la lee el jugador. **Remedido el 19 sep 2026, y no es lo que parecía: no es probabilístico, es una sola letra de un solo reparto.** Renderizando la crónica entera de doce valles fundados con la pareja, el hueco sale en **12 de 12**, siempre la misma plantilla y la misma opción —`feud_inherited.give_b_the_smithy`— y siempre entre los años 26 y 27. La causa exacta: esa plantilla reparte `{ as: 'B', childOf: 'A' }`, y **`childOf` es el único reparto del catálogo que no saca de la reserva de nombrados** (`anyNamed` sí; `childOf` coge un hijo, y un hijo no está nombrado por construcción). Es el único sitio de todo el catálogo que usa `childOf`. **Y eso cambia la decisión de arriba**: «que el reparto sólo coja a gente nombrada» no arregla esta plantilla, la **mata** —su B es un hijo por definición, así que dejaría de ser elegible para siempre—, mientras que «nombrar a quien sale elegido» sí la arregla y no pierde contenido. Sigue siendo decisión del dueño, pero ya no son dos caminos equivalentes |
