...
| Blender · qué viene y su encaje | El agente de Codex está haciendo **el aldeano base, el herrero, el cura y un granjero**, y el dueño confirmó el 16 sep que **se meterán en el juego sustituyendo a los actuales**. Los tres primeros encajan uno a uno. **El granjero no tiene oficio en el motor**: los oficios son jefe, herrero, comadrona, cura, guardabosques, alguacil y herbolario, y la mayoría de la gente no tiene ninguno —son los brazos del campo y usan el modelo base—. Así que el granjero **es** el base, y hay que elegir: o sustituye al base del todo (y entonces críos y viejos también son granjeros), o el base se queda para críos y mayores y el granjero es el adulto sin oficio, que es lo que yo haría y es una línea en `VILLAGER_BY_ROLE` (`renderer.ts`). **Se decide cuando estén las mallas, no antes.** |
