...
| **En vuelo ahora** | nada. **UI-R0 a UI-R6 completas, pero la piel no es la de los prototipos** (comparado el 16 sep: `artifacts/graphics/comparativa/`). Lo siguiente es la tanda **UI-V, la piel**: `ui-redesign/piel/plan-piel.md` (rondas UI-V0 a UI-V6, para Sonnet, con hex y px medidos en los prototipos) y su encargo de arte `ui-redesign/piel/encargo-arte-piel.md` (para la sesión de Blender/Codex, con respaldo para cada activo). Se lanza cuando el dueño diga cómo procede |
...
| **UI-V0 a UI-V6 · la piel** | **pendiente** — el diagnóstico y el plan están en `ui-redesign/piel/plan-piel.md` §1 (por qué salió la estructura y no el aspecto: el prompt prohibía copiar, los tokens se sembraron del juego viejo, no había criterio visual de hecho ni activos ni fuentes empaquetadas) | — |
