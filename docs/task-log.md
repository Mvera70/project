...
| **En vuelo ahora** | **UI-R3** (crónica) y **UI-R4** (personas y fichas), dos agentes Sonnet en paralelo, `src/ui/` salvo `title.ts`/`hud.ts`/`orders.ts`, `index.html`, `tests/fast/ui*`; no tocar. **UI-R2 aterrizada.** Comete el coordinador |
