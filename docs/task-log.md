...
| **VZ · la voz del valle: un sitio, una cola, una piel** | **plan escrito, sin construir**: `ui-redesign/piel/plan-voz.md`, con seis briefs en formato §17 para el carril `construir` y uno de `medir`. Sale de la corrección del dueño del diseño a UI-V10: «esta forma de arreglarlo me parece una chapuza; estos mensajes que salen por encima no cuadran con la interfaz; haz un plan para reestructurarlo, mira cómo son los circuitos». El inventario mide **nueve circuitos** (aviso, cartela, pista, actividad, órdenes, píldora, parte de bienvenida, epitafio, encrucijada), **tres piezas flotando** con tres geometrías, **tres temporizadores y dos arbitrajes** que no se conocen, y **dos pantallas enteras sin vestir** (el parte y el epitafio: cero clases `skin-`, tokens de U-01). La estructura correcta ya estaba escrita en `visual-reference/README.md` §5 («no conservar una quinta cartela flotante») y en el propio comentario U-09 de `app.ts`, y nadie la construyó. El diseño objetivo: **la bandeja enseña una frase de altura fija elegida por una cola pura** (hito > suceso > pista > estado, caducidad por reloj de pared en `paint`, sin `setTimeout`), nada transitorio flota, y con la altura fija nada de lo de encima vuelve a moverse. **Tres decisiones del dueño pendientes** (§3.4): el acento del hito (hoja en oro), la decisión aplazada como sello en el ornamento, y «the line below» en la pista. El anclaje de UI-V10 (`--ui-stack-height` + canto) queda **provisional** hasta VZ-02 |  |
