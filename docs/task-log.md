...
| **UI-V1 a UI-V6 · las pantallas** | **V1, V1b y V2 en `main`; V3 en vuelo** — el diagnóstico y el plan están en `ui-redesign/piel/plan-piel.md` §1 (por qué salió la estructura y no el aspecto: el prompt prohibía copiar, los tokens se sembraron del juego viejo, no había criterio visual de hecho ni activos ni fuentes empaquetadas) | `197b143`, `e38c053`, `c1ddc72` |
...
| **Pendiente de UI-V2b** | mover la frase de actividad y la línea de órdenes dentro de la bandeja (`hud.ts` + `shell.ts`), y arreglar `.valley-hud-right{bottom:60px}` en `index.html` para que los círculos de velocidad no queden ~100 px bajos | — |
