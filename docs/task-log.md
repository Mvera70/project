...
| **A4 bloquea dos filas baratas** | Sol (motor) | **C3** (la atalaya como obra normal de §7.3) y **A5** (la fase del valle en la cabecera) están las dos marcadas `Depende de: A4`, la villa de piedra, que no está hecha. Se intentaron el 18 sep y se pararon ahí, que es lo que la skill de ronda manda. **De C3 se puede salvar la mitad**: el edificio `watchtower` existe y ya se da por el carro (C1), así que pasarlo a la cola de §7.3 con su puerta medida en horas **no necesita A4**; lo que sí la necesita son las torres como mejora del anillo, que es lo que A4 describe. Si se quiere C3 antes de A4, hay que partir la fila en dos |
